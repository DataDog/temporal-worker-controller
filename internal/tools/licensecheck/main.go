...
func execute() error {
	fmt.Println("Creating license classifier")
	...
	for _, lib := range libs {
		fmt.Println("Processing", lib.Name())
	...
	for k, v := range overrides {
		component, ok := components[k]
		if !ok {
			components[k] = v
			continue
		}
		if v.Origin != "" {
			component.Origin = v.Origin
		}
		if v.Copyright != "" {
			component.Copyright = v.Copyright
		}
		if v.License != "" {
			component.License = v.License
		}
		components[k] = component
	}

	if err := writeComponents(f, components); err != nil {
		return err
	}

	fmt.Println("Successfully updated", f.Name())
	return nil
}
